...
3. `ssh -L 51822:127.0.0.1:51822 -L 51821:127.0.0.1:51821 user@ip`
4. Перейти в браузере по `http://127.0.0.1:51822` в Uptime Kuma. Создать новый монитор с типом **Push** и задать ему имя `Home-Server`. Это необходимо, чтобы локальный контейнер `heartbeat_to_vps` смог считать его токен и начать ежеминутную отправку отчетов о жизнеспособности локального сервера.
5. Перейти в браузере по `http://127.0.0.1:51821` в WireGuard панель. Cоздать админ пользователя и всех клиентов (в зависимости от того сколько устройств планируется подключить к VPN).
